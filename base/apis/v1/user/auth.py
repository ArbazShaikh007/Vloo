...
from base.common.utils import push_notification,upload_photos_local,upload_photos,delete_photos
from base.apis.v1.user.models import User,token_required,ProviderSlots
from base.common.helpers import get_normal_message
from  base.apis.v1.admin.models import Store
from base.common.utils import get_hourly_slots
    ...
    def post(self):
        try:
            ...
            if validate_user:
                ...
                validate_user.token_version = (validate_user.token_version or 0) + 1
                validate_user.last_login = datetime.utcnow()
                validate_user.device_token = device_token
                validate_user.device_type = device_type
                db.session.commit()

                token = jwt.encode({'id': validate_user.id, 'token_version': validate_user.token_version, 'role': role,
                ...
                if role == "Customer":
                    ...
                    token = jwt.encode(
                        {'id': add_customer.id, 'token_version': add_customer.token_version, 'role': role,
                         'exp': datetime.utcnow() + timedelta(days=365)},
                        os.getenv("SECRET_KEY"))
                    ...
                    #         if len(slot_list) > 0:
                    #             for j in slot_list:
                    #                 get_slot_data = ProviderSlots.query.filter(ProviderSlots.user_id == add_customer.id,
                    #                                                            ProviderSlots.day == i.day,
                    #                                                            ProviderSlots.start_time == j['start_time'],
                    #                                                            ProviderSlots.end_time == j[
                    #                                                                'end_time']).first()
                    #                 if not get_slot_data:
                    #                     add_new_slots = ProviderSlots(user_id=add_customer.id, day=i.day,
                    ...
                    return jsonify(
                        {'status': 0, 'message': "Account not found with this mobile number. Please contact your provider."})
